...
function removeArtist(artistsArray, number) {
  ...
  artistsArray.splice(number, 1);
  copiedArray = [...artistsArray];
...
function lotsOfArt(array) {
  ...
  for(let i = 0; i < array.length; i++){
    if(array[i].paintings > 100){
      hundredArray.push(array[i].name);
      // hundredArray = [...array];
    }
  }
  return hundredArray;
...
function artistByCountry(array, givenNationality){
  let nationalityArray = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].nationality === givenNationality){
